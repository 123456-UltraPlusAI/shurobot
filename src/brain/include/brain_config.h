#pragma once
/**
 * 存储大脑所需的配置值。这些值应在初始化期间确认，并在机器人的决策过程中保持只读。
 * 在决策过程中需要更改的值应放在BrainData中。
 * 从配置文件读取的原始值，游戏参数，相机分辨率和角度，头部旋转限制，速度限制，策略参数等
 * 从配置文件读取的原始值，游戏参数，相机分辨率和角度，头部旋转限制，速度限制，策略参数等
 *
 * 注意：
 * 1. 配置文件将从config/config.yaml读取。
 * 2. 如果存在config/config_local.yaml，则其值将覆盖config/config.yaml中的值。
 */
#include <string>
#include <ostream>
